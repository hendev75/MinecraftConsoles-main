	...
	*/
	...
	if (screen == nullptr && !ui.GetMenuDisplayed(iPad) )
	{
		...
		*/
		...
		if((player->ullButtonsPressed&(1LL<<MINECRAFT_ACTION_INVENTORY)) && gameMode->isInputAllowed(MINECRAFT_ACTION_INVENTORY) && !gameMode->getLocalPlayerMode()->isSpectator())
		{
		...
		if((player->ullButtonsPressed&(1LL<<MINECRAFT_ACTION_CRAFTING)) && gameMode->isInputAllowed(MINECRAFT_ACTION_CRAFTING) && !gameMode->getLocalPlayerMode()->isSpectator())
		{
			shared_ptr<MultiplayerLocalPlayer> player = Minecraft::GetInstance()->player;
				...
				*/
				...
				if (screen != null) {
					...
					if (Keyboard.getEventKey() == options.keyBuild.key) {
						if (!gameMode->getLocalPlayerMode()->isSpectator())
