...
float GameRenderer::getFov(float a, bool applyEffects)
{
	...
	if (applyEffects)
	{
		...
		// Apply the sprint/bow FOV modifier as a fixed offset based on the standard 70-degree FOV,
		// so the effect stays constant regardless of the FOV slider position
		float modifier = oFov[playerIdx] + (this->fov[playerIdx] - oFov[playerIdx]) * a;
		fov += 70.0f * (modifier - 1.0f);
	}
