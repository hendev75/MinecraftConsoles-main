...
bool PlayerList::placeNewPlayer(Connection *connection, shared_ptr<ServerPlayer> player, shared_ptr<LoginPacket> packet)
{
	...
	// If the player is in spectator mode (e.g. hardcore death), tell the client
	if (player->gameMode->getGameModeForPlayer() == GameType::SPECTATOR)
	{
		playerConnection->send(std::make_shared<GameEventPacket>(GameEventPacket::CHANGE_GAME_MODE, GameType::SPECTATOR->getId()));
	}
