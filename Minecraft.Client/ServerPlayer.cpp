...
void ServerPlayer::readAdditionalSaveData(CompoundTag *entityTag)
{
	...
	if (entityTag->contains(L"playerGameType"))
	{
		...
		// Restore spectator mode in hardcore worlds so it persists across save/load
		if (level != nullptr && level->getLevelData() != nullptr && level->getLevelData()->isHardcore())
		{
			GameType *savedGameType = GameType::byId(entityTag->getInt(L"playerGameType"));
			if (savedGameType == GameType::SPECTATOR)
			{
				gameMode->setGameModeForPlayer(savedGameType);
			}
		}
	}
...
void ServerPlayer::addAdditonalSaveData(CompoundTag *entityTag)
{
	...
	// 4J Stu - We do not want to change the game mode for the player, instead we let the server override it globally
	// Exception: save spectator mode for hardcore worlds so it persists
