...
void Gui::render(float a, bool mouseFree, int xMouse, int yMouse)
{
	...
	// if tooltips are off (or KBM is active), set the y offset to zero
	bool tooltipsHidden = (app.GetGameSettings(iPad,eGameSetting_Tooltips)==0);
#ifdef _WINDOWS64
	tooltipsHidden = tooltipsHidden || g_KBMInput.IsKBMActive();
#endif
	if(tooltipsHidden && bDisplayGui)
	{
		switch(minecraft->player->m_iScreenSection)
		{
		case C4JRender::VIEWPORT_TYPE_FULLSCREEN:
	...
	{
		// In spectator mode, hide the entire HUD (hotbar, crosshair, items, etc.)
		bool bIsSpectator = (minecraft->localgameModes[iPad] != nullptr &&
		                     minecraft->localgameModes[iPad]->getLocalPlayerMode()->isSpectator());
...
// 		}
	...
	{
		bool bIsSpectatorXP = (minecraft->localgameModes[iPad] != nullptr &&
		                       minecraft->localgameModes[iPad]->getLocalPlayerMode()->isSpectator());
	if (bDisplayGui && !bIsSpectatorXP && minecraft->gameMode->hasExperience() && minecraft->player->experienceLevel > 0)
	{
		if (true)
		{
			bool blink = false;
