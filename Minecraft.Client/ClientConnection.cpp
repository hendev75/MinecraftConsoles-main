...
void ClientConnection::handleLogin(shared_ptr<LoginPacket> packet)
{
	...
	if( m_userIndex == ProfileManager.GetPrimaryPad() )
	{
		...
		bool isHardcoreWorld = (MinecraftServer::getInstance() != nullptr && MinecraftServer::getInstance()->isHardcore());
		Level *dimensionLevel = minecraft->getLevel( packet->dimension );
		if( dimensionLevel == nullptr )
		{
			level = new MultiPlayerLevel(this, new LevelSettings(packet->seed, GameType::byId(packet->gameType), false, isHardcoreWorld, packet->m_newSeaLevel, packet->m_pLevelType, packet->m_xzSize, packet->m_hellScale), packet->dimension, packet->difficulty);
	...
	else
	{
		...
		if(level==nullptr)
		{
			...
			bool isHardcoreWorld2 = (MinecraftServer::getInstance() != nullptr && MinecraftServer::getInstance()->isHardcore());
			MultiPlayerLevel *dimensionLevel = new MultiPlayerLevel(this, new LevelSettings(packet->seed, GameType::byId(packet->gameType), false, isHardcoreWorld2, packet->m_newSeaLevel, packet->m_pLevelType, packet->m_xzSize, packet->m_hellScale), packet->dimension, packet->difficulty);
