...
static Minecraft* InitialiseMinecraftRuntime()
{
	...
	InstallDS4XInputHook(); // Hook XInputGetState to inject DS4 HID data
	g_KBMInput.Init();
	DefineActions();
					   ...
					   _In_ int       nCmdShow)
{
	...
	while( WM_QUIT != msg.message && !app.m_bShutdown)
	{
		...
		DS4HIDPoll(); // Read DS4 HID report before InputManager polls XInput
		InputManager.Tick();
		...
		// Periodically detect controller type (Xbox vs PlayStation)
		// Start counter at 119 so the first detection runs on the second frame
		{
			static int s_controllerTypeCheckCounter = 119;
			if (++s_controllerTypeCheckCounter >= 120) // Check every ~2 seconds at 60fps
			{
				s_controllerTypeCheckCounter = 0;
				g_KBMInput.SetControllerType(DetectControllerType());
			}
		}
