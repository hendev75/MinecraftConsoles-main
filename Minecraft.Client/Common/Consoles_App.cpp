...
#endif
{
	...
	app.SetGameHostOption(eGameHostOption_Hardcore, 0);
...
void CMinecraftApp::HandleXuiActions(void)
{
	...
	for(int i=0;i<XUSER_MAX_COUNT;i++)
	{
		...
		if(eAction!=eAppAction_Idle)
		{
			...
			case eAppAction_SpectateRespawn:
				{
					// Hardcore death: respawn the player then switch to spectator mode
					...
					// Switch the player to spectator mode — strips all interaction, enables flight
					if (pMinecraft->localgameModes[i] != nullptr)
					{
						pMinecraft->localgameModes[i]->setLocalMode(GameType::SPECTATOR);
					}
				}
				break;
