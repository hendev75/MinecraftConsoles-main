...
UIScene_DeathMenu::UIScene_DeathMenu(int iPad, void *initData, UILayer *parentLayer) : UIScene(iPad, parentLayer)
{
	...
	// Check if this is a hardcore world
	if (pMinecraft != nullptr && pMinecraft->level != nullptr)
	{
		LevelData *levelData = pMinecraft->level->getLevelData();
		if (levelData != nullptr && levelData->isHardcore())
		{
			m_bIsHardcore = true;
		}
	}
	...
	if (m_bIsHardcore)
	{
		// Hardcore death: show "Spectate World" and "Title Menu" only (no respawn)
		m_buttonRespawn.init(L"Spectate World", eControl_Spectate);
		m_buttonExitGame.init(app.GetString(IDS_EXIT_GAME), eControl_ExitGame);
		m_labelTitle.setLabel(L"You Died!");
	}
	else
	{
...
void UIScene_DeathMenu::handlePress(F64 controlId, F64 childId)
{
	...
	{
	case eControl_Spectate:
		m_bIgnoreInput = true;
		app.SetAction(m_iPad, eAppAction_SpectateRespawn);
		break;
