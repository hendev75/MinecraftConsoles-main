...
#ifdef _WINDOWS64
static bool ReadHardcoreFromSaveFile(const char *utf8SaveFilename, bool &outHardcore, int &outGameType)
{
	outHardcore = false;
	outGameType = 0;
	...
	mbstowcs(wFilename, utf8SaveFilename, MAX_SAVEFILENAME_LENGTH - 1);
	...
	if (fileSize < 12 || fileSize == INVALID_FILE_SIZE) { CloseHandle(hFile); return false; }
					...
					if (off >= 12 && off + len <= saveSize && len > 0 && len < 4 * 1024 * 1024)
					{
						...
						if (root != nullptr)
						{
							...
							if (dataTag != nullptr)
							{
								...
								outGameType = dataTag->getInt(L"GameType");
								found = true;
							}
							delete root;
						}
...
UIScene_LoadMenu::UIScene_LoadMenu(int iPad, void *initData, UILayer *parentLayer) : IUIScene_StartGame(iPad, parentLayer)
{
	...
	else
	{
		...
		if(params->saveDetails != nullptr && params->saveDetails->pbThumbnailData && params->saveDetails->dwThumbnailSize > 0)
		{
			bool bHostOptionsRead = false;
			unsigned int uiHostOptions = 0;
			char szSeed[50];
			ZeroMemory(szSeed,50);
			app.GetImageTextData(params->saveDetails->pbThumbnailData, params->saveDetails->dwThumbnailSize, (unsigned char *)&szSeed, uiHostOptions, bHostOptionsRead, m_MoreOptionsParams.dwTexturePack);
			...
			if(bHostOptionsRead)
			{
				...
				m_bIsHardcore = app.GetGameHostOption(uiHostOptions,eGameHostOption_Hardcore)>0;
				...
				if(app.GetGameHostOption(uiHostOptions,eGameHostOption_FriendsOfFriends) && !(m_bMultiplayerAllowed && (app.GetGameSettings(m_iPad,eGameSetting_Online)!=0)))
				{
		...
		// Fallback: read hardcore and game type from the save file's level.dat
		if(params->saveDetails != nullptr && params->saveDetails->UTF8SaveFilename[0] != '\0')
		{
			bool saveHardcore = false;
			int saveGameType = 0;
			if(ReadHardcoreFromSaveFile(params->saveDetails->UTF8SaveFilename, saveHardcore, saveGameType))
			{
				if(saveHardcore)
				{
					m_bIsHardcore = true;
				}
				// If thumbnail metadata didn't provide a game type, use the one from level.dat
				if(m_iGameModeId == GameType::SURVIVAL->getId() && !m_bGameModeCreative)
				{
					switch(saveGameType)
					{
					case 1:
		...
		if(m_bIsHardcore)
		{
			m_labelCreatedMode.setLabel( L"Created in Hardcore mode" );
			m_buttonGamemode.setLabel(L"Hardcore");
			m_iGameModeId = 3;
			m_bGameModeCreative = false;
		}
		else if(m_bHasBeenInCreative)
		{
			m_labelCreatedMode.setLabel( app.GetString(IDS_CREATED_IN_CREATIVE) );
		}
#endif
...
void UIScene_LoadMenu::tick()
{
	...
	if( m_bSaveThumbnailReady )
	{
		...
		if(bHostOptionsRead)
		{
			...
			else if(app.GetGameHostOption(uiHostOptions,eGameHostOption_HasBeenInCreative)>0)
			{
				m_labelCreatedMode.setLabel( app.GetString(IDS_CREATED_IN_CREATIVE) );
			}
			else
...
//	app.SetGameNewWorldSize(0, false );
	...
	app.SetGameHostOption(eGameHostOption_GameType, pClass->m_bIsHardcore ? GameType::SURVIVAL->getId() : pClass->m_iGameModeId );
	app.SetGameHostOption(eGameHostOption_Hardcore, pClass->m_bIsHardcore ? 1 : 0);
