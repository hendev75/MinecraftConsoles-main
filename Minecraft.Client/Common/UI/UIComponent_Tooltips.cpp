...
#include "..\..\Windows64\PS4TooltipRenderer.h"
#endif
...
#ifdef _WINDOWS64
	// Hide controller tooltips when keyboard and mouse is the active input
	if (g_KBMInput.IsKBMActive())
	{
		return;
	}

	// When a PlayStation controller is detected, render PS4-style button icons
	// instead of the Xbox SWF tooltips.
	if (g_KBMInput.IsPlayStationController())
	{
		// Gather tooltip state for the PS4 renderer
		bool tooltipShow[eToolTipNumButtons];
		wstring tooltipText[eToolTipNumButtons];
		for (int i = 0; i < eToolTipNumButtons; i++)
		{
			tooltipShow[i] = m_tooltipValues[i].show;
			tooltipText[i] = m_tooltipValues[i].label.c_str();
		}

		// Calculate opacity matching the SWF tooltip opacity logic
		unsigned char ucAlpha = app.GetGameSettings(ProfileManager.GetPrimaryPad(), eGameSetting_InterfaceOpacity);
		float fOpacity;
		if (ucAlpha < 80)
		{
			...
			if (uiOpacityTimer != 0)
			{
				...
				{
					float fStep = (80.0f - static_cast<float>(ucAlpha)) / 10.0f;
					fOpacity = 0.01f * (80.0f - ((10.0f - static_cast<float>(uiOpacityTimer)) * fStep));
				}
				else
					fOpacity = 0.01f * 80.0f;
			}
			else
				fOpacity = 0.01f * static_cast<float>(ucAlpha);
		}
		else
			fOpacity = 0.01f * static_cast<float>(ucAlpha);

		g_PS4Tooltips.Render(width, height, tooltipShow, tooltipText, fOpacity);
		return;
	}
#endif
