...
void UIScene_CreateWorldMenu::handlePress(F64 controlId, F64 childId)
{
	...
	case eControl_GameModeToggle:
		...
		case 1: // Adventure
			...
			m_bGameModeCreative = false;
			break;
		case 2: // Hardcore (marker id=3, mapped to Survival at creation time)
...
void UIScene_CreateWorldMenu::CreateGame(UIScene_CreateWorldMenu* pClass, DWORD dwLocalUsersMask)
{
	...
	bool isHardcore = (pClass->m_iGameModeId == 3);
	app.SetGameHostOption(eGameHostOption_Hardcore, isHardcore ? 1 : 0);
	app.SetGameHostOption(eGameHostOption_GameType, isHardcore ? GameType::SURVIVAL->getId() : pClass->m_iGameModeId);
