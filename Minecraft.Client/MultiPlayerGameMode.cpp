...
{
	return localPlayerMode->isSurvival() && !localPlayerMode->isSpectator();
}

bool MultiPlayerGameMode::destroyBlock(int x, int y, int z, int face)
{
	if(localPlayerMode->isSpectator()) return false;
...
{	
	if(localPlayerMode->isSpectator()) return;
...
{
	if(localPlayerMode->isSpectator()) { if(pbUsedItem) *pbUsedItem = false; return false; }
